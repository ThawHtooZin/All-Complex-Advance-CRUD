...
<html lang="en" dir="ltr">
  ...
  <body>
    ...
    if($_POST){
      ...
      $stmt = $pdo->prepare("UPDATE advance1_crud SET username='$username', password='$password', email='$email', role='$role' WHERE id=$id");
      $stmt->execute();
      echo "<script>alert('Updated Successfuly!'); window.location.href='index.php';</script>";
    }
    ?>
    <?php
    $stmt = $pdo->prepare("SELECT * FROM advance1_crud WHERE id=$id");
    $stmt->execute();
